...
class LoadOptimization:
    ...
    def plot_offline(self, Load, Fig3_L):
        ...
        plt.bar(index, Fig3_L[:, 0], bar_width, color='lightgrey', label='Uncontrolled')
        plt.bar(index + bar_width, Fig3_L[:, 1], bar_width, color='grey', label='Agg')
    ...
    def main(self):
        ...
        Load = fac * self.load[23:47]  # Adjust indexing for Python
        Load = Load + 0.1 * np.mean(Load) * np.random.rand(24)
        ...
        L_uncont, X_uncont = self.agg2(BL, 1, 0, 20, 20, fac)

        Fig3_L = np.vstack((1.03 * BL, L_1_1_1000)).T
        self.plot_offline(L_uncont, Fig3_L)
      ...
      #  print("Max L_uncont = ", np.max(L_uncont))
       # print("XDec = ", X_1_1_1000)
        ...
        print("Uncontrolled Load Variance: ", np.var(L_uncont))
